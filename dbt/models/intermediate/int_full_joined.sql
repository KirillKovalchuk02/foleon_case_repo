...
WITH deals_journey AS (
    ...
    WHERE is_won_deal = 1
        AND event_date BETWEEN DATE_SUB(deal_closed, INTERVAL 90 DAY) AND deal_closed --lookback window
),
...
touches_with_costs AS (
    ...
    LEFT JOIN ads_union AS au
        ON LOWER(dj.traffic_source_source) LIKE CONCAT('%', LOWER(au.campaign_name), '%')
),
